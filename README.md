...
- [Quick Start: Incremental Update (No Dolt Required)](#quick-start-incremental-update-no-dolt-required)
...
<small><i><a href='http://ecotrust-canada.github.io/markdown-toc/'>Table of contents generated by markdown-toc</a></i></small>
...
# Quick Start: Incremental Update (No Dolt Required)

**Skip the 10+ hour Dolt clone.** Use `incremental_update.py` to directly append new trading days to your existing qlib_bin data from Tushare API.
...
Get a Tushare token from https://tushare.pro/

## One-time Setup

1. Download the latest `qlib_bin.tar.gz` from [GitHub Releases](https://github.com/chenditc/investment_data/releases)
2. Extract to your qlib data directory:
...
```bash
export TUSHARE=<your_token>
python incremental_update.py --qlib_dir ~/.qlib/qlib_data/cn_data
```

**What it does:**
1. Reads the last date from `calendars/day.txt`
2. Fetches exact trading days from Tushare `trade_cal` API (no wasted API calls on holidays)
3. Downloads daily price + adjustment factors for new trading days
4. Appends data to each stock's `.bin` files (maintains qlib normalize coordinate system)
5. Updates `calendars/day.txt` and `instruments/*.txt`
6. Detects stock splits/dividends (adj_factor changes)

**Features:**
- **Idempotent**: safe to run multiple times — won't create duplicate data
- **Split/dividend detection**: tracks adj_factor changes across trading days
- **New stocks**: automatically creates bin files for newly listed stocks
- **Index constituents**: updates CSI300/500/800/1000/All index weight files

**Data mapping** (reverse-engineered from qlib normalize):

| qlib bin field | Calculation |
|---|---|
| adjclose | `raw_close × tushare_adj_factor` |
| close/open/high/low | `adj_price × scale` (scale from existing bin tail) |
| volume | `tushare_vol / adj_factor` |
| amount | `tushare_amount` (direct) |
...
2. Add price download script. Change [tushare/dump_index_eod_price.py](https://github.com/chenditc/investment_data/blob/main/tushare/dump_index_eod_price.py) to add the index price. Eg. [Example Commit](https://github.com/chenditc/investment_data/commit/ae7e0066336fc57dd60d13b20ac456b5358ef91b)
